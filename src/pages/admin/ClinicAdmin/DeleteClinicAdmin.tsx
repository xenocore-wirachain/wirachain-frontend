...
function DeleteClinicAdmin() {
  ...
  const [deleteClinicAdministration, { isLoading }] =
    useDeleteClinicAdminMutation()
  ...
  const handleDelete = () => {
    if (typeof idSelected === "string") {
      void deleteClinicAdministration(idSelected)
        .unwrap()
        .then(() => {
        ...
        .catch((error: unknown) => {
          ...
          toastRef.current?.show({
            severity: "error",
            summary: "Error",
            detail: `No se pudo eliminar administrador de clinica: ${errorMessage}`,
          })
        })
    } else {
      toastRef.current?.show({
        ...
        summary: "Error",
        detail:
          "ID no válido: se requiere un UUID para eliminar el administrador",
      })
    }
  }
